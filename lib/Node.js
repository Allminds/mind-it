...
App.Node.updateChildTree = function(parent, childTreeName, updatedChildTree) {
  var treeName = App.Node.isRoot(parent) ? childTreeName : "childSubTree",
      childTree = updatedChildTree ? updatedChildTree : (treeName ? parent[treeName] : null),
      set = {};
  if(!childTree || !treeName) return;
  childTree = childTree.map(function(child){
                return child._id;
              });
  set[treeName] = childTree;
...
App.Node.verticalReposition = function(selectedNode, keyPressed) {
  ...
  var parent = selectedNode.parent,
      siblings = (App.Node.isRoot(parent) ? parent[App.Node.getDirection(selectedNode)] : parent.childSubTree) || [],
  ...
  if(App.Node.isRoot(parent)) {
    subListName = App.Node.getDirection(selectedNode);
  }
  subTree[subListName] = siblingIdList;
  mindMapService.updateNode(parent._id, subTree);
};

App.Node.horizontalReposition = function(selectedNode, keyPressed, toggleCallback) {
  //var selectedNode = App.Node.d3NodeToDbNode(selected);
  if(App.Node.isRoot(selectedNode))
        return false;
  ...
  if(repositionDoneFlag === true) {
    ...
    App.Node.updateParentIdOfNode(selectedNode);
    App.Node.updateChildTree(newParent, destinationDirection, destinationSubTree);
