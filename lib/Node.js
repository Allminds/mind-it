...
var findNewParent = null;

App.Node.horizontalReposition = function(selectedNodes, keyPressed, toggleCallback, destinationIndex) {
    if(selectedNodes.filter(App.Node.isRoot).length > 0)
        return false;
    var newParent = null,
        selectedNodeId = selectedNodes.map(function(_){return _._id;}),
        ...
        selectedSiblings = siblings.filter(function(_) {
            return selectedNodeId.indexOf(_._id) != -1;
        });

    if(selectedNodes.filter(function(_){return parent._id != _.parent._id}).length > 0) return false;
